...
- **Docker container health row** ✅ Done 2026-03-21 — `synology_monitor.py` calls `docker ps` to count total and running containers. Driver publishes `containersTotal` / `containersRunning` attributes and adds a single "x of y running" line (with emoji dot) to the `nasSummary` tile. 🟢 = all running, 🟡 = some down, 🔴 = all down, ⚪ = unavailable.

- **Phase 1 expanded health monitor** ✅ Done 2026-03-22 — Added `expandedSummary` attribute for a full-health dashboard tile. New data collected: swap usage, system uptime, per-interface network throughput (shared 1-second sleep with CPU delta), per-container names + state, RAID health from `/proc/mdstat`, and drive SMART pass/fail via `smartctl`. Driver gains `swapPercent`, `uptimeDays` scalar attributes and `swapWarn`/`swapCrit` preferences. `nasSummary` compact tile unchanged.
